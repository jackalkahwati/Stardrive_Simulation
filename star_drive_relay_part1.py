...
class StarDriveRelay:
    ...
    def calculate_lorentz_force(self, magnetic_field, velocity):
        if isinstance(magnetic_field, u.Quantity) and magnetic_field.ndim == 1:
            magnetic_field = np.array([0, 0, magnetic_field.value]) * magnetic_field.unit
        if isinstance(velocity, u.Quantity) and velocity.ndim == 1:
            velocity = np.array([0, 0, velocity.value]) * velocity.unit
        return const.e.si * np.cross(velocity, magnetic_field)
    ...
    def calculate_plasma_interaction(self, velocity, magnetic_field, electric_field):
        ...
        if isinstance(electric_field, u.Quantity) and electric_field.ndim == 1:
            electric_field = np.array([0, 0, electric_field.value]) * electric_field.unit
    ...
    def calculate_lorentz_force_acceleration(self, ring_index, time):
        if self.is_pulsing(time):
            current_density = self.current_densities[ring_index]
            magnetic_field = self.magnetic_fields[ring_index] * np.array([0, 0, 1])  # Convert scalar magnetic field to 3D array
            velocity = np.array([0, 0, self.initial_velocity.value]) * u.m / u.s
            lorentz_force = self.calculate_lorentz_force(magnetic_field, velocity)
            acceleration = lorentz_force / self.spacecraft_mass
    ...
    def calculate_electric_force_acceleration(self, ring_index):
        electric_field = self.electric_fields[ring_index]
        acceleration = (electric_field * const.e.si) / const.m_e.si
        return acceleration.to(u.m / u.s**2)
    ...
    def apply_g_force_constraints(self, acceleration, manned):
        if manned:
            g_force_limit = self.g_force_manned * self.g
        else:
            g_force_limit = self.g_force_unmanned * self.g

        constrained_acceleration = np.minimum(acceleration, g_force_limit)
